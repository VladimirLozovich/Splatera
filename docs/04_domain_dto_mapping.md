# 1. Domain-модели (Внутренние сущности Rust)

В данном проекте "Domain" — это структуры данных, которые использует Rust-бэкенд для логики и сохранения в локальную базу данных (`db.json` / `SQLite`).
...
1. Цветовая палитра:
    - Массив dominant_colors содержит от 0 до 5 цветов.
    - Формат цвета: Строгий HEX (#RRGGBB).
    - Для текстовых файлов массив пуст.
2. Теги:
    - Теги хранятся в нижнем регистре (kebab-case предпочтителен для поиска).
    - Максимальная длина тега: 30 символов.
    - Дубликаты тегов внутри одного Asset запрещены.
3. Превью (Thumbnail):
    - Максимальная ширина/высота генерируемого превью: 600px (достаточно для сетки).
    - Формат превью: WebP (для оптимизации места) или JPEG (качество 80).
4. Файловая система:
    - Приложение не перемещает исходные файлы.
    - Если исходный файл удален, `original_path` остается в БД, но при попытке доступа флаг валидности сбрасывается (UI показывает "Broken Link").
